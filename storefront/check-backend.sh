...
if [ -z "$MEDUSA_BACKEND_URL" ]; then
  echo "Error: MEDUSA_BACKEND_URL is not set."
  echo "Please set this environment variable in your Railway service settings."
  exit 1
fi

echo "Target Backend URL: $MEDUSA_BACKEND_URL"
...
COUNT=0
until curl -f -s -o /dev/null "$MEDUSA_BACKEND_URL/health"; do
  echo "Backend ($MEDUSA_BACKEND_URL) is not ready. Waiting 30 seconds... ($((COUNT+1))/$MAX_RETRIES)"
  sleep 30
  COUNT=$((COUNT+1))
